...
//    mode = "test";
    if(mode == "train"){
        Mat img = cv::imread(prefix+"case0/templ/12.jpg", cv::IMREAD_GRAYSCALE);
        ...
        Mat test_img = imread(prefix+"case0/3.png", cv::IMREAD_GRAYSCALE);
        ...
        for(size_t i=0; i<top5; i++){
            ...
            cv::Point pts1(x - templ[0].width / 2, y - templ[0].height / 2);
            cv::Point pts2(x + templ[0].width / 2, y + templ[0].height / 2);
...
//    mode = "test";
    if(mode == "train"){
        Mat img = imread(prefix+"case1/train.jpg");
        assert(!img.empty() && "check your img path");
